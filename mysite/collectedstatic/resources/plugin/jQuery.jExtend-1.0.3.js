/*
* jQueryExtend-1.0.3.js /jLayer/jDrag/jStyle/Hashtable/jContextmenu/jSwap/jLoadSwf
* 更新了IE6下的兼容性行，目前IE6下暂时还不支持拖动缩放
* name:xiaojia 小嘉
* email:iatt@qq.com
* qq:273142650
* time:2012.6.1 16:50 六一拉~
*
* 2012.12.30 14:20 我把对你的思念写到源代码里，因为我想像光的速度一样立刻飞到你身边。
*/
...
jQuery.fn.extend({
	...
	jDrag: function (obj, json) {
		...
		var StartEventFn = function (e) {
			...
			masks = jQuery('<div style="z-index:' + zIndex + '"></div>').appendTo(document.body);
...
jQuery.extend({
	...
	jLayer: {
		...
		getPosition: function (box, config) {
			...
			var arr = config.find('add:[key=margin]').attr('value').split(/\s+/ig);
		...
		style: function (key, box, json) {
			...
			this.Event(key, box, config, json, changeSize);
		...
		Event: function (key, box, config, json, changeSize) {
			...
			box.find(config.find('add:[key=drag]').attr('value')).jDrag(box, {
				...
				up: function () {
					...
					if (key == 'layer') {
						...
						$(box).find(config.find('add:[key=body]').attr('value')).children('*').css('display', '');
		...
		bodyHeight: function (box, config) {
			...
			if (config.find('add:[key=height]').attr('value') != 'auto') {
		...
		custom: function (key, json) {

			var obj = this.show(key, json);
